...
@api_view(['GET'])
def college_announcements(request, college_id):
    announcements = Post.objects.filter(is_announcement=True, college_id=college_id).order_by('-created_at')
...
@api_view(['GET'])
@permission_classes([AllowAny])
def populate_wormie_content_view(request):
    """
    Temporary endpoint to populate Wormie's content on production.
    In a real app, this would be protected by admin-only permission.
    """
    from django.contrib.auth import get_user_model
    User = get_user_model()
    
    # 1. Update or Create Wormie user
    ...
    # 2. Update image if available in media/user_images
    # (Assuming the file was deployed or exists)
    wormie.image = 'user_images/wormie_profile.png'
    wormie.save()
    ...
    return Response({
        "status": "success",
        "message": f"Created {count} new posts for Wormie. Total posts now exist."
    }, status=status.HTTP_200_OK)
