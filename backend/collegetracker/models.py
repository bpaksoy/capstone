...
class ChatMessage(models.Model):
    # If user is null, it's a guest message (we might want to support this or not)
    # For now, let's keep it tied to a user for "Memory"
...
class Service(models.Model):
    advisor = models.ForeignKey(User, on_delete=models.CASCADE, related_name='services')
    ...
    def __str__(self):
        return f"{self.title} by {self.advisor.username}"
...
class Meeting(models.Model):
    ...
    advisor = models.ForeignKey(User, on_delete=models.CASCADE, related_name='meetings_as_advisor')
    ...
    def save(self, *args, **kwargs):
        if not self.room_name:
            import uuid
            self.room_name = f"wormie-{uuid.uuid4().hex[:12]}"
